...
function equal(entry1, entry2) {
  ...
  const keys = Object.keys(entry1).filter(
    (k) => !['repeated', 'timestamp'].includes(k)
  )
...
const log = function (state = '', action) {
  const tabId =
    action._sender && action._sender.tab ? action._sender.tab.id : null
  switch (action.type) {
    case 'APPEND_LOG_ENTRIES':
      return state
        .concat(
          action.entries.map((e) =>
            Object.assign({}, { tabId, repeated: 0 }, e)
          )
        )
        .reduce((c, e) => {
