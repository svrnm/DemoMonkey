  ...
  },
  {
    ...
    command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
      const iframeCode =
        '<head><title>' +
        parameters[1] +
        '</title><style>html {height:100%;}</style></head><body style="margin:0;padding:0;width:100%;height:100%;overflow:hidden;"><iframe src="' +
        value +
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          return new Group([
            ...
            new Hide(
              parameters[0],
              3,
              'ads-home-list-action-item',
              '',
              'AD_HOME_OVERVIEW',
              location,
              function (node, parentNode) {
                return (
                  node.parentElement.hasAttribute('ad-test-id') &&
                  node.parentElement
                    .getAttribute('ad-test-id')
                    .includes('home-screen-application-card-application-name')
                )
              }
            )
          ])
      ...
      },
      {
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          return new Hide(
            parameters[0],
            9,
            'ads-analytics-business-outcomes-card-size',
            '',
            'ANALYTICS_BUSINESS_OUTCOMES',
            location
          )
        }
      },
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          ...
          return new Group([
            new OverwriteHTML(
              ...
              'ad-screenshot-tile-stitcher[ad-container-width="500"] > div',
              screenshotHtml,
              location
            ),
            new OverwriteHTML(
              'EUM_MOBILE_SESSION_DETAILS',
              'ad-screenshot-tile-stitcher[ad-container-width="202"] > div',
              thumbnailHtml,
              location
            )
          ])
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          if (
            ...
            return new Group([
              ...
              new ReplaceNeighbor(
                parameters[0],
                'images/tierTypes/' + imageName + '.svg',
                2,
                'g.adsNodeCountContainer image',
                '',
                '',
                location,
                (search, replace, node) => {
          ...
          if (typeof value === 'string' && value.includes(',')) {
            ...
            return new Group([
              new ReplaceNeighbor(
                ...
                tierCount,
                2,
                'g.adsApplicationNode text.adsNodeCountText:nth-of-type(1)',
                '',
                '',
                location
              ),
              new ReplaceNeighbor(
                parameters[0],
                parseInt(tierCount) === 1 ? 'TIER' : 'TIERS',
                2,
                'g.adsApplicationNode text.adsNodeCountTextSmall:nth-of-type(2)',
                '',
                '',
                location
              ),
              new ReplaceNeighbor(
                ...
                nodeCount,
                2,
                'g.adsApplicationNode text.adsNodeCountText:nth-of-type(3)',
                '',
                '',
                location
              ),
              new ReplaceNeighbor(
                parameters[0],
                parseInt(nodeCount) === 1 ? 'NODE' : 'NODES',
                2,
                'g.adsApplicationNode text.adsNodeCountTextSmall:nth-of-type(4)',
                '',
                '',
                location
              )
          ...
          return new Group([
            new ReplaceNeighbor(
              ...
              value,
              2,
              'g.adsTierNode text.adsNodeCountText',
              '',
              '',
              location
            ),
            new ReplaceNeighbor(
              parameters[0],
              parseInt(value) === 1 ? 'Node' : 'Nodes',
              2,
              'g.adsTierNode text.adsNodeCountTextSmall',
              '',
              '',
              location
            ),
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          ...
          if (
            ...
            parameters[1] = {
              ...
              warning: '.adsWarningNodeColor',
              critical: '.adsCriticalNodeColor'
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          ...
          if (value && ['normal', 'warning', 'critical'].includes(value.toLowerCase())) {
            value = {
              ...
              critical: 'images/health/critical.svg'
            }[value.toLowerCase()]
          }
          return new ReplaceNeighbor(
            parameters[0],
      ...
      },
      {
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          ...
          const commands = [
            ...
            cmdBuilder._buildCustomCommand(
              'appdynamics',
              'replaceInnerNodeHealth',
              [parameters[0]],
              value[3]
            ),
            cmdBuilder._buildCustomCommand(
              'appdynamics',
              'replaceOuterNodeHealth',
              [parameters[0]],
              value[4]
            )
      ...
      },
      {
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          return new SearchAndReplace(
            ...
            value,
            'APP_BT_LIST',
            'tr td:nth-child(3) .x-grid-cell-inner',
            '',
            location
          )
        }
        ...
        command: function (value, parameters, location, includeRules, excludeRules, cmdBuilder) {
          ...
          if (typeof value[1] === 'string' && value[1] !== '') {
            commands.unshift(
              cmdBuilder._buildCustomCommand(
                'appdynamics',
                'replaceBusinessTransactionOriginalName',
                [parameters[0]],
                value[1]
              )
            )
          ...
          if (typeof value[1] === 'string' && value[2] !== '') {
            commands.unshift(
              cmdBuilder._buildCustomCommand(
                'appdynamics',
                'replaceBusinessTransactionHealth',
                [parameters[0]],
                value[2]
              )
            )
