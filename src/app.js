...
const initPopup = async () => {
  ...
  let commitHash = ''
  try {
    const r = await fetch(window.chrome.runtime.getURL('COMMITHASH'))
    commitHash = await r.text()
  } catch (e) {
  ...
  try {
    ...
    try {
      const configuration = await protocolHandler.handle(window.location.search)
      if (configuration) {
        const configurations = store.getState().configurations
        await store.dispatch({ type: 'ADD_CONFIGURATION', configuration })
